...
function pesquisar() {
    const termoPesquisa = document.querySelector(".barra-pesquisa").value.toLowerCase();
...
function exibirResultadosPesquisa(resultados) {
    ...
    if (resultados.length === 0 || !document.querySelector(".barra-pesquisa").value.trim()) {
        ulResultadosPesquisa.style.display = "none";
    } else {
        resultados.forEach(resultado => {
            const nomeProduto = resultado.querySelector(".produto-nome").textContent;
            const idProduto = resultado.querySelector(".ver-produto").getAttribute("data-product-id");
...
botaoLupa.addEventListener("click", () => {
    ...
    if (headerWidth < 380) {
        ...
        logoAluraGeek.style.display = logoAluraGeek.style.display === "none" ? "block" : "none";
    }
});